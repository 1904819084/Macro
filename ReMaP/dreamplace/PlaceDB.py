...
class PlaceDB (object):
    ...
    def initialize_from_rawdb(self, params):
        ...
        self.node_names = np.array(pydb.node_names, dtype=string_dtype)
        ...
        if (not params.global_place_flag) and os.path.exists(params.aux_input):
            ...
            if filename is not None and os.path.exists(filename):
                ...
                self.node_orient = np.zeros(self.num_physical_nodes, dtype=string_dtype)
        ...
        if not use_read_pl_flag:
            ...
            self.node_orient = np.array(pydb.node_orient, dtype=string_dtype)
        ...
        self.pin_direct = np.array(pydb.pin_direct, dtype=string_dtype)
        ...
        self.pin_names = np.array(pydb.pin_names, dtype=string_dtype)
        self.net_name2id_map = pydb.net_name2id_map
        self.pin_name2id_map = pydb.pin_name2id_map
        self.net_names = np.array(pydb.net_names, dtype=string_dtype)
...
""" % (
        ...
        if params.macro_criterion == "top_k":
            self.movable_macro_mask = np.zeros(self.num_movable_nodes, dtype=bool)
            self.movable_macro_mask[np.argsort(movable_node_area)[-params.macro_top_k_criterion_k:]] = True
        elif params.macro_criterion == "area":
            mean_movable_node_area = self.total_movable_node_area / self.num_movable_nodes
            self.movable_macro_mask = np.zeros(self.num_movable_nodes, dtype=bool)
            self.movable_macro_mask[:self.num_movable_nodes] = \
                (movable_node_area > mean_movable_node_area * params.macro_area_criterion_area_ratio) \
                & (self.node_size_y[:self.num_movable_nodes] > self.row_height * params.macro_area_criterion_size_y_row_height_ratio)
        else:
    ...
    def write(self, params, filename, sol_file_format=None, blockages=None):
        """
        ...
        if not params.legalize_flag and not params.detailed_place_flag and sol_file_format == place_io.SolutionFileFormat.BOOKSHELF:
            self.write_pl(params, temp_filename, node_x, node_y)
        else:
            place_io.PlaceIOFunction.write(self.rawdb, temp_filename, sol_file_format, node_x, node_y)
        
        if params.remap_flag:
            macro_ids = np.where(self.movable_macro_mask)[0]
            fixed_macro_name = self.node_names[macro_ids]
            # pdb.set_trace()
            process_def.fix_certain_macros(
                file_path=temp_filename, macro_names=fixed_macro_name
            )
            if blockages is None:
                replace_flag = False
                blockage_ls = []
                if params.blockage_size_x[0] > 1e-5 and params.blockage_size_y[0] > 1e-5 and \
                   params.blockage_size_x[1] > 1e-5 and params.blockage_size_y[1] > 1e-5:
                    for macro_id in macro_ids:
                        # macro_id = self.node_name2id_map[macro_name.decode('utf-8')]
                        macro_size_x = self.node_size_x[macro_id] / params.scale_factor
                        macro_size_y = self.node_size_y[macro_id] / params.scale_factor
                        blockage_size_x = macro_size_x + (self.xh - self.xl) / params.scale_factor * params.blockage_size_x[0] / params.blockage_size_x[1]
                        blockage_size_y = macro_size_y + (self.yh - self.yl) / params.scale_factor * params.blockage_size_y[0] / params.blockage_size_y[1] 
                        center_x = node_x[macro_id] + macro_size_x / 2
                        center_y = node_y[macro_id] + macro_size_y / 2
                        lower_left_coord = (center_x - blockage_size_x / 2, center_y - blockage_size_y / 2)
                        upper_right_coord = (center_x + blockage_size_x / 2, center_y + blockage_size_y / 2)
                        blockage_ls.append([lower_left_coord[0], lower_left_coord[1], upper_right_coord[0], upper_right_coord[1]])
            else:
                replace_flag = True
                blockage_ls = blockages
            process_def.modify_blockages(file_path=temp_filename, blockages=blockage_ls, replace=replace_flag)
